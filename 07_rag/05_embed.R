...
if(!file.exists(".env") && file.exists("../.env")) readRenviron("../.env")
...
sqlite_vec = import("sqlite_vec")

## 0.2 Configuration ##########################

# Path to sqlite-vec extension from the active Python environment.
VEC_EXTENSION_PATH = sqlite_vec$loadable_path()

DB_PATH = if(file.exists("07_rag/data")) "07_rag/data/embed.db" else "data/embed.db" # path to your new vector embeddings database
if(file.exists(DB_PATH)) { unlink(DB_PATH, force = TRUE) }
DOCUMENT = if(file.exists("07_rag/data")) "07_rag/data/lower_manhattan_recovery_plan.txt" else "data/lower_manhattan_recovery_plan.txt" # path to text doc
...
embed_model = NULL
get_embed_model = function() {
  if (is.null(embed_model)) embed_model <<- st$SentenceTransformer(EMBED_MODEL)
  embed_model
}
...
conn = dbConnect(RSQLite::SQLite(), DB_PATH, loadable.extensions = TRUE)
