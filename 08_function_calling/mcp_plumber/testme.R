...
# Call the run_linear_regression tool directly
regression_result <- mcp_request("tools/call", list(
...
cat("Direct regression tool output:\n")
mcp_text_block(regression_result) %>% fromJSON() %>% print()
...
messages <- create_message(role = "user",
  content = "In mtcars, run a simple linear regression with mpg as the response and wt as the predictor.")
