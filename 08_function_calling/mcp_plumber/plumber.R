...
TOOLS <- list(
  ...
  list(
    ...
    description = "Run a simple linear regression using one numeric response column and one numeric predictor column from a dataset.",
    inputSchema = list(
      type       = "object",
      properties = list(
        dataset_name = list(
        ...
        response = list(
          type        = "string",
          description = "Numeric response column to predict, such as 'mpg'."
        ),
        predictor = list(
          type        = "string",
          description = "Numeric predictor column to use, such as 'wt'."
        )
      ),
      required = list("dataset_name", "response", "predictor")
    )
  )
)
...
run_tool <- function(name, args) {
  ...
  if (name == "run_linear_regression") {
    ...
    if (!response %in% names(df)) {
      stop(paste("Unknown response column:", response))
    }
    if (!predictor %in% names(df)) {
      stop(paste("Unknown predictor column:", predictor))
    }
    if (!is.numeric(df[[response]])) {
      stop(paste("Response column must be numeric:", response))
    }
    if (!is.numeric(df[[predictor]])) {
      stop(paste("Predictor column must be numeric:", predictor))
    }

    formula_text <- paste(response, "~", predictor)
    fit <- lm(stats::as.formula(formula_text), data = df)
    fit_summary <- summary(fit)

    coefficient_table <- as.data.frame(fit_summary$coefficients)
    ...
    result <- list(
      ...
      n_obs = nrow(stats::model.frame(fit)),
      r_squared = round(fit_summary$r.squared, 4),
      adjusted_r_squared = round(fit_summary$adj.r.squared, 4),
      residual_standard_error = round(fit_summary$sigma, 4),
